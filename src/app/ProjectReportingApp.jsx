...
const VIEW_THEMES = {
  ...
  'document-creator': {
    page: 'bg-[radial-gradient(circle_at_top_left,#eef2ff_0,#f8faff_34%,#fbfdff_68%,#ffffff_100%)]',
  ...
  return (
    ...
    >
      ...
      <main className="relative z-[1] mx-auto max-w-7xl px-4 py-7 md:px-6 md:py-8">
        ...
        {mainView === 'document-creator' && (
          <section className="overflow-hidden rounded-3xl border border-white/90 bg-white/[0.94] shadow-[0_20px_60px_-40px_rgba(15,23,42,0.55)] ring-1 ring-slate-900/[0.08]">
            <div className="flex flex-col gap-3 border-b border-indigo-100 bg-indigo-50/70 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <h2 className="text-xl font-bold tracking-tight text-slate-900">Tvorba dokumentů</h2>
                <p className="mt-1 text-xs text-slate-600">Původní portál institucí a generátor listin je plně začleněný do projektového výkaznictví.</p>
              </div>
              <a
                href={DOCUMENT_CREATOR_URL}
                target="_blank"
                rel="noreferrer"
                className="inline-flex items-center justify-center gap-2 rounded-xl border border-indigo-200 bg-white px-4 py-2.5 text-sm font-bold text-indigo-900 shadow-sm transition hover:bg-indigo-50"
              >
                Otevřít samostatně
                <ExternalLink className="h-4 w-4" />
              </a>
