...
test('ISIR AI používá frontu a původní oddělený tok shrnutí, formulářů a kazuistiky', () => {
  ...
  const individualCheckFlow = appSource.slice(
    appSource.indexOf('const verifyClientFromIsirView'),
    appSource.indexOf('const verifySelectedClientInsolvency')
  );
  ...
  assert.match(
    appSource,
    /document_ids:\s*mode === 'case-study' && contextDocuments\.length[\s\S]*new Set\(contextDocuments\.map/
  );
  assert.match(appSource, /ověřuji, zda běží lhůta pro podávání přihlášek/);
  assert.match(individualCheckFlow, /if \(!options\.silent && hasCaseDocuments\)/);
  assert.match(individualCheckFlow, /await runAutomaticIsirAi\(snapshot, client\)/);
  assert.doesNotMatch(individualCheckFlow, /if \(matched &&/);
  assert.doesNotMatch(automaticFlow, /deadlineWasLeftUnverified/);
  ...
  assert.match(isirSource, /formatClaimsDeadlineStatus/);
  assert.match(isirSource, /Skončila \$\{formatDate\(deadline\)\}/);
  assert.match(isirSource, /progressNotice \|\| 'Kontrola nyní neběží\.'/);
});
